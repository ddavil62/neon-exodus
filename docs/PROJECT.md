...
| 자동 사냥 | IAP 영구 해금, AI 자동 이동, 우선순위 기반, 인게임 자동 레벨업 토글 | 완료 |
...
| 레벨업 오버레이 | 3택 카드, 리롤, 전체 완료 시 스킵, 인게임 자동 레벨업 토글 | 완료 |
| 결과 화면 | 무기별 리포트, 난이도 보상, DC 분배 연출, 콘텐츠 압축 | 완료 |
| 진화 힌트 팝업 | 무기 Max(Lv8) 도달 시 조합 유물 안내 팝업 모달, 큐 기반 순차 표시 | 완료 |
| 진화/엔들리스 모달 | 게임 일시정지 + 정보 패널 | 완료 |
| 설정/통계 | [설정]/[통계] 탭 전환, 3섹션 카드(사운드/게임플레이/시스템)+슬라이드 토글+도트 그리드 배경+2단계 확인 데이터 초기화, 통계(전체/스테이지별/캐릭터별) | 완료 |
