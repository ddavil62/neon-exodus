...
│   └── iap-real-billing.spec.js    # IAP 실제 Billing 연동 테스트 (16개)
└── docs/
    ├── PROJECT.md                 # 이 문서
    ├── CHANGELOG.md               # 변경 이력
...
- 롱탭 설명 툴팁: 스킬 행(투자 버튼 제외) 500ms 롱탭 시 오버레이 표시
  - 감지 zone: 270x28 depth 0, pointermove |dy|>5px 시 취소
  - 패널: depth 1000, 280px, COLORS.UI_PANEL alpha 0.95, NEON_CYAN 1px 테두리, cornerRadius 8
  - lv=0: levels[0].descKey + "(Lv.1 미리보기)" 안내
  - lv>=1: levels[lv-1].descKey 현재 레벨 설명
  - blocker zone: depth 999, 360x640, pointerdown + pointerup 모두 _hideSkillTooltip() (BUG-1 수정)
...
Google Play 인앱결제를 통한 유료 기능 해금. `@capgo/native-purchases` v8 플러그인으로 실제 Google Play Billing을 처리한다. 웹 환경에서는 Mock 모드 동작.

#### IAPManager
- 싱글톤 패턴 (AdManager와 동일 구조)
- `initialize()`: `window.Capacitor.Plugins.NativePurchases` 전역 참조로 플러그인 로드. `isBillingSupported()` 확인 후 `getProduct()`로 상품 정보 프리로드. 실패 시 Mock 모드 폴백
- `purchase(productId)`: `purchaseProduct({ productIdentifier, productType: 'inapp', quantity: 1 })` 호출. Google Play 결제 다이얼로그 표시. Mock 모드에서는 `{ purchased: true }` 즉시 반환. 사용자 취소 시 `{ purchased: false, error: 'cancelled' }` 반환 (실패 토스트 미표시). 기타 실패 시 `{ purchased: false, error }` 반환. `isBusy` 플래그로 중복 호출 차단
- `restorePurchases()`: `getPurchases({ productType: 'inapp' })` 호출. Mock 모드에서는 `{ restored: false }` 반환. 네이티브에서 `purchase.productIdentifier`로 autoHunt 상품 확인 시 SaveManager에 기록
- `getLocalizedPrice()`: 프리로드된 상품 정보에서 현지화 가격 문자열 반환. 없으면 `'$ 0.99'` 폴백
...
| 패키지 | 버전 | 비고 |
|---|---|---|
| `@capgo/native-purchases` | `^8.2.2` | Capacitor 8 전용, Google Play Billing Library 7.x 내장 |
...
2. 구매 버튼 클릭 -> IAPManager.purchase() -> Google Play 결제 다이얼로그 -> 성공 시 IAPManager.unlockAutoHunt() + 씬 새로고침
...
- 스펙 문서: `.claude/specs/2026-03-09-auto-hunt.md`, `.claude/specs/2026-03-12-iap-real-billing.md`
