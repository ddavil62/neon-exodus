...
| 33 | 버튼 | Graphics 사각형 | 네온 테두리 버튼 SVG (9-slice) | 미착수 |
| 34 | 카드 배경 | Graphics 패널 | UI 카드 SVG (9-slice) | 미착수 |
...
### Phase 3: 스테이지 에셋 (멀티 스테이지 시스템) -- 미착수
> 4스테이지 시스템에 필요한 배경 타일, 보스, 무기 아이콘

- [ ] 배경 타일 3종 (128x128, S2~S4 전용 바닥 타일, GPT Image API)
- [ ] 신규 보스 3종 (128x128, siege_titan_mk2 / data_phantom / omega_core, GPT Image API)
- [ ] 신규 무기 아이콘 4종 (32x32, force_blade / nano_swarm / vortex_cannon / reaper_field, SVG 직접 생성)

**산출물**: `assets/backgrounds/` 3종, `assets/sprites/bosses/` 3종, `assets/ui/icons/` 4종
**에셋 수**: 10종 (GPT Image API 6종 + SVG 직접 생성 4종)

### Phase 4: 무기 이펙트 (최종 폴리싱)
...
**산출물**: `assets/sprites/effects/` 무기 이펙트 벡터 에셋
**예상 에셋 수**: ~10종
...
| Phase 3 | 10종 | 미착수 | 스테이지 에셋 (배경 3 + 보스 3 + 무기 아이콘 4) |
| Phase 4 | ~10종 | 미착수 | 무기 이펙트 |
| **합계** | **~71종** | - | GPT Image API + SVG 하이브리드 생성 |

---

*최종 수정: 2026-03-10 (Phase 3 스테이지 에셋 10종 추가 — 멀티 스테이지 시스템)*
